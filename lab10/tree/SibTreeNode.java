    ...
    // FILL IN YOUR SOLUTION TO PART II HERE.
	  if (isValidNode()) {
		  ...
		  if (c <= 1 || children() == 0) {
			  ...
			  SibTreeNode childNode = this.firstChild;
			  int i = 1;
			  while(i < c - 1) {
				  childNode = childNode.nextSibling;
				  i++;
			  }
			  newChild.nextSibling = childNode.nextSibling;
			  childNode.nextSibling = newChild;
		  }
		  // update size
		  myTree.size++;
	  }else {
	  ...
	  if (isValidNode()) {
		  if (children() == 0) {
			  if (this.parent != null) {
				  ...
				  if (kid != this && kid.nextSibling != this) {
					  kid = kid.nextSibling;
				  }
				  if (kid == this) {		// this is the first node in the tree
